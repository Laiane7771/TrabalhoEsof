...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "agenda" because\n';
  ...
  $wnd.__gwt_activeModules['agenda'] = {
    'moduleName' : 'agenda',
    'bindings' : function() {
      return {};
    }
 ...
 * "/agenda.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['agenda'].superdevmode = true;

  var expectedSuffix = '/agenda.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + agenda + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:agenda';
  var devModeSessionKey = '__gwtDevModeSession:agenda';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/agenda/agenda.nocache.js';
  ...
  $wnd.__gwt_activeModules['agenda'].canRedirect = true;
  $wnd.__gwt_activeModules['agenda'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/agenda?user.agent=' + ua + '&_callback=' + callback;
